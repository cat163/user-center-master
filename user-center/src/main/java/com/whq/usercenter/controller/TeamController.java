...
import com.whq.usercenter.common.DeleteRequest;
...
import com.whq.usercenter.model.request.TeamAddRequest;
...
public class TeamController {
    ...
    public BaseResponse<Team> getTeamById(long id){
        ...
        if (team == null){
            throw new BusinessException(ErrorCode.NULL_ERROR, "查询无该队伍");
        }
    ...
    public BaseResponse<List<TeamUserVO>> listMyJoinTeam(TeamQuery teamQuery, HttpServletRequest request){
        ...
        List<UserTeam> userTeamList = userTeamService.list(userTeamQueryWrapper);
        // 取出不重复的队伍 id
        //List<Long> TeamIdList = userTeamList.stream().distinct().collect(Collectors.toList());
        Map<Long, List<UserTeam>> listMap = userTeamList.stream().collect(Collectors.groupingBy(UserTeam::getTeamId));
        // 相同teamId的UserTeam分成一组，并且把teamId作为Key
